...
const LearningJourneySection = () => {
  ...
  // --- FIX: Add state for growth mindset, initialized with fallback data ---
  const [growthMindset, setGrowthMindset] = useState(portfolioData.growthMindset);
    ...
    const fetchAllData = async () => {
      try {
        ...
        if (journeyRes.success && journeyRes.data) {
          setLearningJourney(journeyRes.data);
        }
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'in-progress': return <Clock className="w-6 h-6 text-yellow-400 animate-spin" />;
      case 'planned': return <Calendar className="w-6 h-6 text-blue-400" />;
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'completed': return <Badge className="bg-green-500/20 text-green-400 border-green-400/50">Completed</Badge>;
      case 'in-progress': return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-400/50">In Progress</Badge>;
      case 'planned': return <Badge className="bg-blue-500/20 text-blue-400 border-blue-400/50">Planned</Badge>;
      default: return <Badge className="bg-gray-500/20 text-gray-400 border-gray-400/50">Unknown</Badge>;
    }
  };
  ...
  return (
    <section id="learning" className="py-20 px-4 relative z-10">
      <div className="max-w-6xl mx-auto">
        {/* ... Your original header and timeline mapping ... */}
        <div className="text-center mb-16">
          ...
          <ShinyText text="Continuous growth and skill development pathway" disabled={false} speed={2} className="max-w-2xl mx-auto font-['Poppins']" />
        </div>
                            ...
                            <Card className={`bg-slate-800/40 backdrop-blur-lg border border-slate-700/50 hover:border-cyan-400/50 transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:shadow-cyan-500/10 group ${index % 2 === 0 ? 'mr-8' : 'ml-8'}`}>
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-between mb-4">
                                        <h3 className="text-xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent font-['Orbitron'] group-hover:text-cyan-400 transition-colors">
                                            {phase.phase}
                                        </h3>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        {phase.skills.map((skill, skillIndex) => (
                                            <Badge key={skillIndex} variant="outline" className="border-slate-600 text-slate-300 hover:border-cyan-400 hover:text-cyan-400 transition-colors">
                                                {skill}
                                            </Badge>
                                        ))}
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <div className="w-full bg-slate-700 rounded-full h-2">
                                            <div className="h-2 rounded-full bg-gradient-to-r from-cyan-400 to-purple-400 transition-all duration-1000" style={{ width: getProgressWidth(phase.status) }}></div>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
        ...
        {/* --- FIX: This section is now dynamic --- */}
          ...
          <Card className="bg-gradient-to-r from-slate-800/40 to-purple-900/20 backdrop-blur-lg border border-slate-700/50 max-w-3xl mx-auto">
            <CardContent className="p-8">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                {/* Use the dynamic title, with a fallback */}
                <h3 className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent font-['Orbitron']">{growthMindset?.title || 'Growth Mindset'}</h3>
              </div>
              <p className="text-slate-300 text-lg font-['Poppins'] leading-relaxed italic">
                {/* Use the dynamic quote, with a fallback */}
                "{growthMindset?.quote || 'Loading quote...'}"
              </p>
            </CardContent>
          </Card>
        </div>
